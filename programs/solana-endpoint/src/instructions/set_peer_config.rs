...
pub struct SetPeerConfig<'info> {
    ...
    #[account(
        ...
        seeds = [PEER_SEED, msg_store.key().as_ref(), &params.remote_eid.to_be_bytes()],
        bump
    )]
    ...
    #[account(
        ...
        bump = msg_store.bump,
        has_one = admin @MsgError::Unauthorized
    )]
        ...
        match params.config.clone() {
            ...
            PeerConfigParam::FeeBps(fee_bps) => {
                if let Some(fee_bps) = fee_bps {
                    require!(fee_bps < MAX_FEE_BASIS_POINTS, MsgError::InvalidFee);
                }
